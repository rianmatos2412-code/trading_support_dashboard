...
export async function fetchSignals(params?: {
  ...
  const signalsResponse = await fetch(`${API_URL}/signals?${queryParams}`);
  ...
  const fallbackResponse = await fetch(`${API_URL}/alerts?${queryParams}`);
  if (!fallbackResponse.ok) throw new Error("Failed to fetch alerts");
